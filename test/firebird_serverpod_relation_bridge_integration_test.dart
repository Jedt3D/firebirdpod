  ...
  factory _AuthorRow.fromJson(Map<String, dynamic> json) {
    ...
    return _AuthorRow(
      ...
      books: books == null
          ? null
          : (books as List)
              .map((entry) => _BookRow.fromJson(Map<String, dynamic>.from(entry as Map)))
              .toList(),
    );
  }
...
class _AuthorInclude extends IncludeObject {
  _AuthorInclude._({
    _CompanyInclude? company,
    _BookIncludeList? books,
  }) : _company = company,
       _books = books;
...
void main() {
  group('Phase 02 Serverpod relation bridge', () {
    ...
    test('find materializes object and list includes across relation graphs', () async {
      if (!shouldRunDirectIntegrationTests()) {
        return;
      }
    ...
    test('findById keeps nullable object includes null when the relation is missing', () async {
      if (!shouldRunDirectIntegrationTests()) {
        return;
      }
    ...
    test('find rejects IncludeList pagination until per-parent windowing lands', () async {
      if (!shouldRunDirectIntegrationTests()) {
        return;
      }
      ...
      await expectLater(
        () => connection.find<_AuthorRow>(
          session,
          include: _AuthorInclude._(
            books: _BookIncludeList._(limit: 1),
          ),
        ),
        throwsA(
          isA<UnsupportedError>().having(
            (error) => error.message,
            'message',
            contains('IncludeList pagination'),
          ),
        ),
      );
    });
