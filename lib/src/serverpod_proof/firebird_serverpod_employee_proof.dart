...
class FirebirdEmployeeProofSerializationManager
  ...
  final internal.Protocol _internalProtocol = internal.Protocol();

  @override
  T deserialize<T>(dynamic data, [Type? t]) {
    t ??= T;
    try {
      return _internalProtocol.deserialize<T>(data, t);
  ...
  @override
  Table? getTableForType(Type t) => _internalProtocol.getTableForType(t);
...
class FirebirdServerpodEmployeeProofApp {
  ...
  static const _runtimeSettingsTableName = 'serverpod_runtime_settings';
  static const _bootstrapRuntimeSettingsTableSql = '''
execute block as
begin
  if (not exists(
    select 1
    from rdb\$relations
    where rdb\$relation_name = 'SERVERPOD_RUNTIME_SETTINGS'
  )) then
    execute statement '
      create table serverpod_runtime_settings (
        id integer generated by default as identity primary key,
        logSettings blob sub_type text character set utf8 not null,
        logSettingsOverrides blob sub_type text character set utf8 not null,
  ...
  if (not exists(
    select 1
    from rdb\$relations
    where rdb\$relation_name = 'SERVERPOD_MIGRATIONS'
  )) then
    execute statement '
      create table serverpod_migrations (
        id integer generated by default as identity primary key,
        module varchar(128) character set utf8 not null,
...
''';
  ...
  Future<void> ensureStartBootstrapSchema() async {
    final session = await pod.createSession(enableLogging: false);
    try {
      await session.db.unsafeExecute(_bootstrapRuntimeSettingsTableSql);
    } finally {
      await session.close();
    }
  }

  Future<int> runtimeSettingsRowCount() async {
    final session = await pod.createSession(enableLogging: false);
    try {
      final result = await session.db.unsafeQuery('''
        select count(*) as runtime_settings_count
        from $_runtimeSettingsTableName
        ''');
      final value = result.first.toColumnMap()['RUNTIME_SETTINGS_COUNT'];
      if (value is int) return value;
      if (value is num) return value.toInt();
      throw StateError('Expected integer runtime settings count.');
    } finally {
      await session.close();
    }
  }

  Future<void> startWithBootstrap() async {
    await ensureStartBootstrapSchema();
    pod.storage.clear();
    await pod.start();
  }
